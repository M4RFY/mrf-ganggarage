	...
	for k, v in pairs(Config.Locations) do
		if v.garage then
			...
			exports['qb-target']:AddBoxZone("GangGarage: "..k, vector3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="GangGarage: "..k, heading = out.w+180.0, debugPoly=Config.Debug, minZ=out.z-1.05, maxZ=out.z+0.80 },
				{ options = { { event = "mrf-garagegang:client:Garage:Menu", icon = "fas fa-clipboard", label = "gang Vehicles", gang = v.gang, coords = v.garage.spawn, list = v.garage.list, prop = Parking[#Parking] }, },
				distance = 2.0 })
		end
	end
end)
...
RegisterNetEvent('mrf-garagegang:client:Garage:Menu', function(data)
	...
	vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = "Close", params = { event = "mrf-garagegang:client:Menu:Close" } }
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-clipboard-list", header = "Vehicle out of Garage",
										...
										params = { event = "mrf-garagegang:client:Garage:Blip", }, }
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = "Remove Vehicle", params = { event = "mrf-garagegang:client:RemSpawn" } }
	else
		...
		for k, v in pairsByKeys(data.list) do
			...
			if showButton == true then
				...
				vehicleMenu[#vehicleMenu+1] = { icon = seticon, header = (data.list[k].CustomName or searchCar(k)), params = { event = "mrf-garagegang:client:SpawnList", args = { spawnName = spawnName, coords = data.coords, list = v } } }
			end
		end
	end
	showButton = nil
...
RegisterNetEvent("mrf-garagegang:client:SpawnList", function(data)
...
RegisterNetEvent("mrf-garagegang:client:RemSpawn", function(data)
...
RegisterNetEvent("mrf-garagegang:client:Garage:Blip", function(data)
